...
export async function processAiMessage(from: string, input: string, session: WaSession, hospitalContext: string): Promise<void> {
  ...
  if (functionCalls && functionCalls.length > 0) {
    ...
    try {
      ...
      // Send the function response back to Gemini to get the final conversational response
      ...
      responseText = '❌ I encountered an error while processing that request.'
    }
  }
  ...
  if (responseText) {
    ...
    if (session.patientUid) {
      try {
        ...
        await saveChatSession(session.patientUid, existingMessages, new Date().toISOString())
      } catch (e) {
